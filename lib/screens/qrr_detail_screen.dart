...
class _QRRDetailScreenState extends State<QRRDetailScreen> {
  ...
  Widget _buildHeader() {
    return Card(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: BorderSide(color: _qrr.priority.color.withOpacity(0.7)),
      ),
      child: Padding(
        padding: const EdgeInsets.all(16),
        child: Column(
          ...
          children: [
            Row(
              children: [
                Icon(
                  _qrr.displayIcon ?? Icons.assignment,
                  color: _qrr.displayColor ?? Colors.blue,
                  size: 32,
                ),
  ...
  FloatingActionButton? _buildActionButton(bool isParticipant, bool canJoin) {    if (_qrr.status == QRRStatus.completed || _qrr.status == QRRStatus.cancelled) {
      return null;
    }
