...
class AuthService extends ChangeNotifier {
  ...
  // Limpa TODOS os dados de cache, incluindo SharedPreferences
  Future<void> _clearAllCaches() async {
    Logger.info("LIMPEZA PROFUNDA: Removendo todos os vestígios de sessão anterior...");
    
    // 1. Limpar dados de autenticação
    _currentUser = null;
    _token = null;
    
    // 2. Limpar armazenamento seguro
    await _secureStorage.deleteAll();
    
    // 3. Limpar SharedPreferences (onde podem estar dados de clãs, federações, etc.)
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.clear();
      Logger.info("SharedPreferences completamente limpo.");
    } catch (e) {
      Logger.warning("Erro ao limpar SharedPreferences: $e");
    }
    
    // 4. Limpar cache da API Service (se houver)
    try {
      _apiService.clearCache();
      Logger.info("Cache da API Service limpo.");
    } catch (e) {
      Logger.warning("Erro ao limpar cache da API Service: $e");
    }
    
    Logger.info("Limpeza profunda concluída. Sistema pronto para nova identidade.");
  }
