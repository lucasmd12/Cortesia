...
class FederationService extends ChangeNotifier {
  ...
  /// Busca todas as federações com seus dados de território para o mapa.
  /// Este método não utiliza paginação para garantir que todos os territórios sejam carregados.
  Future<List<Federation>> getFederationsWithTerritories() async {
    _isLoading = true;
    notifyListeners();
    try {
      // O endpoint pode ser o mesmo, mas garantimos que a resposta do backend inclua os territórios.
      final response = await _apiService.get('/api/federations?limit=1000', requireAuth: true); // Limite alto para buscar todos

      if (response != null && response['success'] == true && response['data'] is List) {
        final List<dynamic> federationsData = response['data'];
        final federationsWithTerritories = federationsData.map((json) => Federation.fromJson(json)).toList();
        Logger.info('Fetched ${federationsWithTerritories.length} federations with territory data.');
        return federationsWithTerritories;
      } else {
        Logger.warning('Unexpected response format when fetching federations with territories: $response');
        return [];
      }
    } catch (e) {
      Logger.error('Error fetching federations with territories: $e');
      rethrow;
    } finally {
      _isLoading = false;
      notifyListeners();
  ...
  /// Aloca um território para um clã dentro de uma federação.
  /// Usado por líderes de federação.
  Future<bool> allocateClanTerritory({
    ...
    final endpoint = '/api/federations/$federationId/allocate-territory/$clanId';
    final body = {
    ...
    try {
      ...
      if (response != null && response['success'] == true) {
        Logger.info('Territory allocated successfully for clan $clanId in federation $federationId.');
        notifyListeners(); // Notifica para atualizar a UI, se necessário
        return true;
      } else {
        Logger.error('Failed to allocate clan territory. Response: $response');
        throw Exception(response['message'] ?? 'Falha ao alocar território do clã.');
      }
    } catch (e) {
      Logger.error('Error allocating clan territory: $e');
      rethrow;
    }
  }
  // ===================== FIM DA MODIFICAÇÃO ======================
