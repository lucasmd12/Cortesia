...
class ClanService with ChangeNotifier {
  ...
  /// Permite que o usuário logado saia do seu clã atual.
  /// A rota não requer um ID de clã, pois o backend o identifica pelo token do usuário.
  Future<void> leaveCurrentClan() async {
    Logger.info('Attempting to leave the current clan.');
    try {
      // A rota é um POST para /api/clans/leave e não precisa de corpo (body).
      await _apiService.post('/api/clans/leave', {}, requireAuth: true);
      Logger.info('Successfully left the clan.');
      // Opcional: notificar a UI para recarregar os dados do usuário ou do clã.
      notifyListeners();
    } catch (e) {
      Logger.error('Error leaving clan: $e');
      // Re-lança a exceção para que a UI possa tratá-la (ex: mostrar um SnackBar).
      rethrow;
    }
  }

  // ==================== INÍCIO DA MODIFICAÇÃO ====================
  /// Busca todos os clãs com seus dados de território para o mapa.
  /// Este método não utiliza paginação para garantir que todos os territórios sejam carregados.
  Future<List<Clan>> getClansWithTerritories() async {
    _setLoading(true);
    try {
      // Usamos um limite alto para buscar todos os clãs de uma vez.
      // O parâmetro `withTerritory=true` pode ser usado para otimizar a query no backend, se implementado.
      final response = await _apiService.get('/api/clans?limit=1000&withTerritory=true', requireAuth: true);

      if (response != null && response['success'] == true && response['data'] is List) {
        final List<dynamic> clansData = response['data'];
        final clansWithTerritories = clansData.map((json) => Clan.fromMap(json)).toList();
        Logger.info('Fetched ${clansWithTerritories.length} clans with territory data.');
        return clansWithTerritories;
      } else {
        Logger.warning('Unexpected response format when fetching clans with territories: $response');
        return [];
      }
    } catch (e, s) {
      Logger.error('Error fetching clans with territories', error: e, stackTrace: s);
      rethrow; // Re-lança para que a UI possa tratar o erro.
    } finally {
      _setLoading(false);
    }
  }
