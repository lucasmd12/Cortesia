...
class SegmentedNotificationService {
  ...
  /// Envia notificação global para todos os usuários (apenas ADMs)
  Future<Map<String, dynamic>> sendGlobalNotification({
    ...
    try {
      ...
      if (response['success'] == true) {
        Logger.info('Global notification sent successfully - sent: ${response['sent']}, failed: ${response['failed']}, total: ${response['total']}');
        
        return {
          'success': true,
          'message': response['message'] ?? 'Notificação global enviada com sucesso',
          'sent': response['sent'] ?? 0,
          'failed': response['failed'] ?? 0,
          'total': response['total'] ?? 0,
          'details': response['details'],
        ...
        Logger.error('Global notification failed - error: ${response['message']}');
        return {
          'success': false,
          'message': response['message'] ?? 'Erro ao enviar notificação global',
          'error': response['error'],
        };
      }
    } catch (error) {
      Logger.error('Error sending global notification: ${error.toString()}');
      return {
        'success': false,
        'message': 'Erro de conexão ao enviar notificação global',
        'error': error.toString(),
      };
    }
  }
  ...
  Future<Map<String, dynamic>> previewAdminRecipients() async {
    try {
      ...
      final response = await _apiService.get('/api/notifications/admin/preview-recipients');
